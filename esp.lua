...
local function updateCornerLines(lines, topLeft, topRight, bottomLeft, bottomRight, boxWidth, boxHeight)
    -- Corner length proportional to box size (max 20% of width/height so lines never touch)
    local cornerX = math.max(2, boxWidth * 0.20)
    local cornerY = math.max(2, boxHeight * 0.20)

    -- Top-left
    lines[1].From = topLeft
    lines[1].To = topLeft + Vector2.new(cornerX, 0)
    lines[2].From = topLeft
    lines[2].To = topLeft + Vector2.new(0, cornerY)

    -- Top-right
    lines[3].From = topRight
    lines[3].To = topRight - Vector2.new(cornerX, 0)
    lines[4].From = topRight
    lines[4].To = topRight + Vector2.new(0, cornerY)

    -- Bottom-left
    lines[5].From = bottomLeft
    lines[5].To = bottomLeft + Vector2.new(cornerX, 0)
    lines[6].From = bottomLeft
    lines[6].To = bottomLeft - Vector2.new(0, cornerY)

    -- Bottom-right
    lines[7].From = bottomRight
    lines[7].To = bottomRight - Vector2.new(cornerX, 0)
    lines[8].From = bottomRight
    lines[8].To = bottomRight - Vector2.new(0, cornerY)
...
RunService.RenderStepped:Connect(function()
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        -- Static aspect-ratio projection for 2D/Corner ESP
        ...
        -- Minimum size threshold to prevent tiny overlapping blocks
        if boxWidth < 6 or boxHeight < 6 then
            hideAll()
            continue
        end
        ...
        if ESP.CornerBoxEnabled then
            local topLeft = boxPos
            local topRight = boxPos + Vector2.new(boxWidth, 0)
            local bottomLeft = boxPos + Vector2.new(0, boxHeight)
            local bottomRight = boxPos + Vector2.new(boxWidth, boxHeight)
